...
检测点 \(S_i\) 的示向度 \(\theta_i\) 并非一条确定射线，而是半张角为 \(1\degree\) 的扇形可行域。Tokekar 和 Isler 将未知但有界的测向误差表示为包含真实目标的二维扇形，并以多个扇形交集的面积或直径度量定位不确定性\cite{tokekar2013bearing}，与本题的交会定位机制一致。将所有扇形与目标圆域求交可得到干扰源定位区域。若交会约束已使区域有界且目标圆边界不活跃，则该区域为凸多边形；其直径必由两个顶点取得。确定直径端点后，再检验其他顶点到直径中点的距离，即可回答直径圆的覆盖问题。
