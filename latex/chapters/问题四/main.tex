...
因此，定向源与全向源的实际数量在样例间随机变化，而非预先固定比例。
全部样例采用同一组控制器参数和预先固定的随机种子。
...
\safeimage[0.99\textwidth]{figures/q4_testset_coverage_and_extreme.pdf}{问题四测试集覆盖与极端构型。（a）源数量为 $10$--$16$、每种数量 $300$ 次的共 $2100$ 个测试样例；蓝色圆点表示全向源，红色三角表示定向源，灰色虚线为 $1800\,\mathrm m$ 目标区域边界。（b）非均匀边界极端样例及模型实际巡检轨迹。\label{fig:q4-extreme-boundary}}

全部 $2100$ 个样例均完成场景内所有源的清除。总体总时间均值为
$6509.2\,\mathrm s$，$95\%$ 分位数为 $7295.8\,\mathrm s$。
源数量由 $10$ 增至 $16$ 时，平均定位清除时间由
$681.2\,\mathrm s$ 降至 $343.8\,\mathrm s$，表明中心--双环巡检、
主干移动与同址多频道检测的时间开销可由更多干扰源共同分担。
图~\ref{fig:q4-extreme-boundary} 展示了该压力样例中的实际巡检轨迹，
其中编号按模型成功清除的先后顺序排列。该单次运行不替代表
\ref{tab:p4-source-count} 的分组统计。

\subsection{正式测试结果}

三次正式测试用于记录控制器在竞赛模拟器中的实际执行结果和接口运行
情况，不用于替代前述 $2100$ 次随机实验对模型总体表现的评价。平均
