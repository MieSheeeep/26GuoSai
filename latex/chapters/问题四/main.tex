...
因此，定向源与全向源的实际数量在样例间随机变化；这一 $0.5$ 概率用于覆盖不同混合比例。全部样例采用同一组控制器参数和预先固定的随机种子。
...
的终止口径。
图~\ref{fig:q4-extreme-boundary} 给出了边界样例中的实际巡检轨迹，
其中编号按模型成功清除的先后顺序排列。
...
三次正式测试用于核实控制器在竞赛模拟器中的实际执行和接口情况。
平均定位清除时间按模拟器返回的总虚拟时间除以成功清除数量计算，
程序运行时间按实际墙钟记录。
